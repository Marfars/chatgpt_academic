...
def ArgsGeneralWrapper(f):
    ...
    def decorated(
        cookies, txt, txt2, top_p, temperature, chatbot, history, system_prompt, *args
    ):
        txt_passon = txt
        if txt == "" and txt2 != "":
            txt_passon = txt2
    ...
    def mt(i_say, history):
        while True:
            try:
                ...
                mutable[
                    1
                ] = f"警告，文本过长将进行截断，Token溢出数：{n_exceed}，截断比例：{(1 - p_ratio):.0%}。"
...
def write_results_to_file(history, file_name=None):
    ...
    if file_name is None:
        ...
        file_name = (
            "chatGPT分析报告" + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".md"
        )
    ...
    def decorated(txt, top_p, temperature, chatbot, history, systemPromptTxt, WEB_PORT):
        try:
            ...
            yield from update_ui(
                chatbot=chatbot, history=history, msg=f"异常 {e}"
            )  # 刷新界面
...
def on_file_uploaded(files, chatbot, txt):
    ...
    for file in files:
        ...
        err_msg += extract_archive(
            f"private_upload/{time_tag}/{file_origin_name}",
