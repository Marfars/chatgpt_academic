...
def 解析Paper(
    ...
    for index, fp in enumerate(file_manifest):
        ...
        if ".pdf" in fp.lower():
            ...
            file_content = (
                BeautifulSoup("".join(file_content), features="lxml")
                .body.text.encode("gbk", "ignore")
                .decode("gbk")
            )
...
def 批量总结PDF文档pdfminer(
    ...
    # 基本信息：功能、贡献者
    chatbot.append(
        ["函数插件功能？", "批量总结PDF文档，此版本使用pdfminer插件，带token约简功能。函数插件贡献者: Euclid-Jie。"]
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
    ...
    if len(file_manifest) == 0:
        report_execption(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何.tex或pdf文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    yield from 解析Paper(
