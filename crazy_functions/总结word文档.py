...
def 解析docx(
    ...
    for index, fp in enumerate(file_manifest):
        ...
        i_say = (
            prefix + f"请对下面的文章片段用中英文做概述，文件名是{os.path.relpath(fp, project_folder)},"
            f"文章内容是 ```{file_content}```"
        )
        i_say_show_user = (
            prefix
            + f"[{index + 1}/{len(file_manifest)}] 假设你是论文审稿专家，请对下面的文章片段做概述: {os.path.abspath(fp)}"
        )
...
def 总结word文档(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    # 基本信息：功能、贡献者
    chatbot.append(["函数插件功能？", "批量总结Word文档。函数插件贡献者: JasonGuo1"])
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
