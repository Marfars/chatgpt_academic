...
def 全项目切换英文(txt, llm_kwargs, plugin_kwargs, chatbot, history, sys_prompt, web_port):
    ...
    file_manifest = [
        ...
        f
        for f in glob.glob("./crazy_functions/*.py")
        if ("test_project" not in f) and ("gpt_log" not in f)
    ]
    ...
    def thread_worker(fp, index):
        ...
        with open(fp, "r", encoding="utf-8", errors="replace") as f:
            file_content = f.read()
        i_say_template = (
            lambda fp, file_content: f"接下来请将以下代码中包含的所有中文转化为英文，只输出代码，文件名是{fp}，文件代码是 ```{file_content}```"
        )
        try:
            gpt_say = ""
            # 分解代码文件
            file_content_breakdown = breakdown_txt_to_satisfy_token_limit(
                file_content, get_token_fn, MAX_TOKEN
            )
    ...
    # 第7步：所有线程同时开始执行任务函数
    handles = [
        threading.Thread(target=thread_worker, args=(fp, index))
        for index, fp in enumerate(file_manifest)
    ]
    for h in handles:
        h.daemon = True
    ...
    while True:
        ...
        for thread_index, alive in enumerate(th_alive):
            observe_win.append(
                "[ ..."
                + observe_window[thread_index][0][-60:]
                .replace("\n", "")
                .replace("```", "...")
                .replace(" ", ".")
                .replace("<br/>", ".....")
    ...
    for index, h in enumerate(handles):
        ...
        chatbot.append(
            (
                i_say_show_user,
                f"[Local Message] 已完成{os.path.abspath(fp)}的转化，\n\n存入{os.path.abspath(where_to_relocate)}",
            )
        )
