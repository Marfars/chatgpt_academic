...
def 全项目切换英文(txt, llm_kwargs, plugin_kwargs, chatbot, history, sys_prompt, web_port):
    ...
    def thread_worker(fp, index):
        ...
        with open(fp, 'r', encoding='utf-8', errors='replace') as f:
            file_content = f.read()
        i_say_template = lambda fp,
                                file_content: f'接下来请将以下代码中包含的所有中文转化为英文，只输出代码，文件名是{fp}，文件代码是 ```{file_content}```'
        try:
            gpt_say = ""
            # 分解代码文件
            ...
            for file_content_partial in file_content_breakdown:
                ...
                # # ** gpt request **
                gpt_say_partial = predict_no_ui_long_connection(inputs=i_say, llm_kwargs=llm_kwargs, history=[],
    ...
    while True:
        ...
        for thread_index, alive in enumerate(th_alive):
            observe_win.append(
                "[ ..." + observe_window[thread_index][0][-60:].replace('\n', '').replace('```', '...').replace(' ',
                                                                                                                '.').replace(
                    '<br/>', '.....').replace('$', '.') + "... ]")
    ...
    for index, h in enumerate(handles):
        ...
        chatbot.append((i_say_show_user,
