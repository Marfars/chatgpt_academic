...
def get_crazy_functions():
    ...
    function_plugins = {
        "解析整个Python项目": {"Color": "stop", "Function": HotReload(解析一个Python项目)},  # 按钮颜色
        "解析整个C++项目头文件": {"Color": "stop", "Function": HotReload(解析一个C项目的头文件)},  # 按钮颜色
        ...
        "读Tex论文写摘要": {"Color": "stop", "Function": HotReload(读文章写摘要)},  # 按钮颜色
        "批量生成函数注释": {"Color": "stop", "Function": HotReload(批量生成函数注释)},  # 按钮颜色
        "[多线程Demo] 解析此项目本身（源码自译解）": {"Function": HotReload(解析项目本身)},
    ...
    function_plugins.update(
        {
            ...
            "批量总结Word文档": {"Color": "stop", "Function": HotReload(总结word文档)},
            "理解PDF文档内容 （模仿ChatPDF）": {
                # HotReload 的意思是热更新，修改函数插件代码后，不需要重启程序，代码直接生效
                "Color": "stop",
                "AsButton": False,  # 加入下拉菜单中
